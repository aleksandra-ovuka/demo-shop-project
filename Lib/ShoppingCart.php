...
class ShoppingCart
{
    ...
    public function reduceQuantity(Product $product, $quantity=1) {
        if ($product instanceof Product) {
            foreach ($this->getItems() as $key => $item) {
                if   ($item->getProduct()->id == $product->id && $item->getQuantity()==1 ) {
                    unset($this->items[$key]);  }
                
                else if ($item->getProduct()->id == $product->id && $item->getQuantity()>1 ) {
                    $item->setQuantity($item->getQuantity()-1);
                    $this->getItems()[]=$item; } }
            return $this->getItems();
             } }
    public function increaseQuantity(Product $product, $quantity=1) {
    if ($product instanceof Product) {
        foreach ($this->getItems() as $key => $item) {
            if   ($item->getProduct()->id == $product->id) {
                $item->setQuantity($item->getQuantity()+$quantity);
